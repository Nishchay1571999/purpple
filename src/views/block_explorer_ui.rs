    ...
    use crate::components::{searchEntry,navbar};

    pub fn create_block_explorer_page() -> Box {
        let vbox = Box::new(Orientation::Vertical, 10);

        let navbar = navbar::create_block_explorer_top_navbar();
        vbox.append(&navbar);

        let search_bar = searchEntry::create_new_search_entry("Search for blocks,Transactions or programs");
